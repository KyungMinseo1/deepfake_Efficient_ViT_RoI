...
real_df = pd.read_csv(os.path.join(CSV_PATH, 'real_path.csv'))
fake_df = pd.read_csv(os.path.join(CSV_PATH, 'fake_path.csv'))
...
def extract_paths(folder_lst, paths, labels):
    """
    paths: list -> [[path, path, ...], [path, path, ...], ]
    labels: list -> [0, 1, 0, 1, ...]
    """
    if folder_lst != 'All':
        sampled_real = real_df[real_df.Data.isin(folder_lst)]
        sampled_fake = fake_df[fake_df.Data.isin(folder_lst)]
    else:
        sampled_real = real_df
        sampled_fake = fake_df

    sampled_real = sampled_real.groupby('Data', group_keys=False).sample(frac=0.5, random_state=42)
    sampled_fake = sampled_fake.groupby('Data', group_keys=False).sample(frac=0.5, random_state=42)

    for idx, row in tqdm(sampled_real.iterrows(), total=len(sampled_real)):
        try:
            data_dir_path = row.Path
            if os.path.isdir(data_dir_path):
                frames_name = os.listdir(data_dir_path)
            subpath_lst = []
            for f_n in frames_name:
                subpath_lst.append(os.path.join(data_dir_path, f_n))
            paths.append(subpath_lst)
            labels.append(0)
        except Exception as e:
            print(f"{data_dir_path} - skipped for {e}")
            continue
    print("Real path saved")

    for idx, row in tqdm(sampled_fake.iterrows(), total=len(sampled_fake)):
        try:
            data_dir_path = row.Path
            if os.path.isdir(data_dir_path):
                frames_name = os.listdir(data_dir_path)
...
if __name__ == "__main__":
    ...
    if opt.dataset == "Sample":
        ...
        extract_paths(folders, paths=paths, labels=labels)

        train_paths, val_paths, train_label, val_label = train_test_split(paths, labels, test_size=0.2)
        ...
        folders = "All"
        print('Target Folders: All')
        extract_paths("All", paths=paths, labels=labels)
        train_paths, val_paths, train_label, val_label = train_test_split(paths, labels, test_size=0.2)
        train_data = list(zip(train_paths, train_label))
        val_data = list(zip(val_paths, val_label))
    ...
    if is_sample:
        ...
        for t in range(starting_epoch, opt.num_epochs + 1):
            ...
            tqdm_train_bar = tqdm(dl, desc=f'EPOCH #{t} (Train)')
            
            for index, (images, coordinates, labels) in enumerate(tqdm_train_bar):
                labels = labels.unsqueeze(1).to(device)
                images = images.to(device)
                coordinates = coordinates.to(device)
                ...
                y_prob = torch.sigmoid(y_pred.detach().cpu()) if y_pred.dim() == 2 else y_pred.detach().cpu()
                train_preds_list.append(y_prob.numpy())
                train_labels_list.append(labels.detach().cpu().numpy())
                
                tqdm_train_bar.set_postfix({
                    'Loss': f'{total_loss / counter:.4f}',
                    'Acc': f'{train_correct / (counter * config["training"]["bs"]):.4f}',
                    '0s': negative,
                    '1s': positive
                }, refresh=False)
            ...
            tqdm_val_bar = tqdm(val_dl, desc=f'EPOCH #{t} (Val)')

            with torch.no_grad():
                for index, (val_images, val_coordinates, val_labels) in enumerate(tqdm_val_bar):
                    ...
                    val_prob = torch.sigmoid(val_pred.cpu()) if val_pred.dim() == 2 else val_pred.cpu()
                    val_preds_list.append(val_prob.numpy())
                    val_labels_list.append(val_labels.cpu().numpy())

                    tqdm_val_bar.set_postfix({
                        'V_Loss': f'{total_val_loss / val_counter:.4f}',
                        'V_Acc': f'{val_correct / (val_counter * val_dl.batch_size):.4f}'   # type: ignore
                    }, refresh=False)
            ...
            all_train_labels = np.concatenate(train_labels_list)
            ...
            train_f1 = f1_score(all_train_labels, train_binary_preds)
            val_f1 = f1_score(all_val_labels, val_binary_preds)
            ...
            print(
                ...
                f"Acc: {train_correct:.4f} "
                f"F1: {train_f1:.4f} "
                f"AUC: {train_auc:.4f} | "
                f"Val Loss: {total_val_loss:.4f} "
                f"Val Acc: {val_correct:.4f} "
                f"Val F1: {val_f1:.4f} "
                f"Val AUC: {val_auc:.4f} "
                f"Val 0s: {val_negative}/{np.count_nonzero(validation_labels == 0)} "
                f"Val 1s: {val_positive}/{np.count_nonzero(validation_labels == 1)}"
            )
...
# 처음: python cross-efficient-vit/cross-efficient-vit/train_show_DF40_v2.py --dataset All --efficient_net 7 --config cross-efficient-vit/cross-efficient-vit/configs/architecture_for_b7.yaml
# 처음(b0): python cross-efficient-vit/cross-efficient-vit/train_show_DF40_v2.py --dataset All --efficient_net 0 --config cross-efficient-vit/cross-efficient-vit/configs/architecture.yaml
# 샘플: python cross-efficient-vit/cross-efficient-vit/train_show_DF40_v2.py --dataset Sample --efficient_net 7 --config cross-efficient-vit/cross-efficient-vit/configs/architecture_for_b7.yaml
# 재개: python cross-efficient-vit/cross-efficient-vit/train_show_DF40_v2.py --dataset All --resume models/efficientnet_checkpoint10_All.pth --efficient_net 7 --config cross-efficient-vit/cross-efficient-vit/configs/architecture_for_b7.yaml